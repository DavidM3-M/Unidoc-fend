...
const GestionAvalesRectoria = () => {
  ...
  // NUEVA FUNCIÓN: Ver Hoja de Vida
  const handleVerHojaVida = async (userId: number) => {
    const url = `${import.meta.env.VITE_API_URL}/vicerrectoria/hoja-de-vida-pdf/${userId}`;
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-col sm:flex-row flex-wrap gap-2">
            {/* NUEVO BOTÓN: Ver Hoja de Vida */}
            <button
              onClick={() => handleVerHojaVida(row.original.id)}
              className="bg-purple-600 text-white px-2 sm:px-3 py-2 rounded-lg hover:bg-purple-700 transition-colors flex items-center justify-center gap-2 text-xs sm:text-sm whitespace-nowrap"
            >
